      ...
      <form onSubmit={handleSavePrices} className="card-container">
        ...
        <div className="services-grid" style={{ marginBottom: '24px' }}>
          ...
          {serviceEntries.map(([key, item]) => {
            ...
            return (
              <div key={key} className="service-item-card active" style={{ position: 'relative' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                  <h3 style={{ fontSize: '1rem', fontWeight: '700', color: 'var(--yellow-primary)' }}>
                    {item.name} {item.isCustom ? '⭐ (Custom Service)' : ''}
                  </h3>

                  {/* DELETE SERVICE BUTTON FOR CUSTOM / ADDITIONAL SERVICES */}
                  {item.isCustom && (
                    <button
                      type="button"
                      onClick={() => handleDeleteService(key, item.name)}
                      style={{
                      ...
                      title="Delete this custom service"
                    >
                      <Trash2 size={13} />
                      <span>Delete</span>
                    </button>
