...
import { Calendar, Clock, User, Phone, Car, Plus, Send, CheckCircle2, Trash2 } from 'lucide-react';
import { addBooking, deleteBooking } from '../utils/storage';
...
export default function Bookings({ bookings, setBookings, currentLang = 'en' }) {
  ...
  const handleDeleteBookingItem = (id, name) => {
    if (window.confirm(`Are you sure you want to cancel & delete booking for "${name}"?`)) {
          ...
          {bookings.map(b => (
            <div key={b.id} className="history-log-row" style={{ padding: '16px 0' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                ...
                <button
                  onClick={() => handleDeleteBookingItem(b.id, b.customerName)}
                  style={{ background: 'transparent', border: 'none', color: 'var(--ruby-primary)', cursor: 'pointer', padding: '6px' }}
